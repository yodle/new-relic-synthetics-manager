...
function readSyntheticContents(syntheticInfo, syntheticFileService, callback) {
    ...
    syntheticFileService.getBase64File(syntheticInfo.filename, (buffer, err) => {
...
class UpdateMonitorOrchestrator {
    ...
    updateSynthetic(syntheticName) {
        ...
        async.waterfall([
            function (next) {
                getSyntheticInfo(this.syntheticListFileService, syntheticName, (syntheticInfo, err) => {
                    next(err, syntheticInfo);
                });
            }.bind(this),

            function (syntheticInfo, next) {
                readSyntheticContents(syntheticInfo, this.syntheticFileService, (newInfo, err) => {
                    next(err, newInfo);
                });
            }.bind(this),

            function (syntheticInfo, next) {
                uploadSyntheticToNewRelic(syntheticInfo, this.newRelicService, (err) => {
                    next(err);
                });
            }.bind(this)
        ], function (err) {
    ...
    updateSyntheticByFilename(syntheticFilename) {
        ...
        async.waterfall([
            ...
            if (err) { throw err; }
            logger.verbose('Updating synthetic complete: ' + syntheticFilename);
        });
    }
}
