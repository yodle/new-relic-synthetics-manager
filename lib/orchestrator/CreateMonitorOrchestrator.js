...
function createSyntheticInNewRelic(
    ...
    async.waterfall([
        function (next) {
            newRelicService.createSynthetic(name, locations, type, frequency, status, function (syntheticUrl, err) {
        ...
        function (id, next) {
            ...
            syntheticsListFileService.addSynthetic(id, name, filename, function (err) {
                next(err);
            });
        }
    ], function (err) {
