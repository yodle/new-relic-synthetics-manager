...
const LOCATIONS_ENDPOINT = '/synthetics/api/v1/locations';
const LOCATIONS_URL = 'https://' + SYNTHETICS_HOST + LOCATIONS_ENDPOINT;
const ALERTS_ENDPOINT = '/synthetics/api/v1/monitors';
...
class SyntheticsService {
    ...
    addAlertEmails(id, emails, callback) {
        logger.verbose('NewRelicService.addAlertEmails: ' + id);
        ...
        logger.debug('require parameters: ' + JSON.stringify(requestParam));

        const options = this._getOptions({
            url: ALERTS_URL + '/' + id + '/notifications',
            method: 'POST',
            body: JSON.stringify(requestParam),
        });
    ...
    removeAlertEmail(id, email, callback) {
        logger.verbose('NewRelicService.removeAlertEmail: ' + id);

        logger.debug('email: ' + email);

        const options = this._getOptions({
            url: ALERTS_URL + '/' + id + '/notifications/' + encodeURIComponent(email),
            method: 'DELETE'
        });
